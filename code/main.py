...
class PaperInformation:
    ...
    def __lt__(self, other: "PaperInformation"):
        self_year, self_month = self.date.split("-")
        other_year, other_month = other.date.split("-")
        if self_year != other_year:
            return int(self_year) > int(other_year)
        if self_month != other_month:
            return int(self_month) > int(other_month)
        if self.venue != other.venue:
            return self.venue < other.venue
        return self.paper < other.paper
...
def main():
    ...
    with open("../outputs/table.md", "w") as f:
